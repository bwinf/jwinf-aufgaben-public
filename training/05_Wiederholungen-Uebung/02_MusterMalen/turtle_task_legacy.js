function initTask(subTask) {
   subTask.gridInfos = {
      ...
      turtleStepSize: 5,
      maxInstructions: {easy: 6, medium: 7, hard: 7},
   ...
   subTask.data = {
      easy: [{
          drawSolution : function(turtle) { for(var i = 0; i < 4; i++) {turtle.move(2); turtle.turn(270);}} ,
         }],
         medium: [{
             drawSolution : function(turtle) {for(var i = 0; i < 4; i++) {for(var j = 0; j < 3; j++) {turtle.move(1); turtle.turn(90);} turtle.move(2);} },
         }],
         hard: [{
             drawSolution : function(turtle) {for(var i = 0; i < 2; i++) {for(var j = 0; j < 3; j++){for(var k = 0; k < 3; k++){turtle.turn(90);} turtle.move(1)} turtle.turn(90);}},
         }],
   };
